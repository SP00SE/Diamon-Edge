...
// ─── CACHE ────────────────────────────────────────────────────────────────
const cache = new Map();
const CACHE_TTL = 10 * 60 * 1000; // 10 minutes — matches the visibility-return refresh cadence; a shorter RAM TTL forced full re-fetches every cycle
...
// Coalesce concurrent cache-misses for the same URL into one network fetch —
// parallel game waves request identical team endpoints (bullpen, IL, roster)
// on doubleheader days, and both racers previously hit the network.
const _inflightFetches = new Map();

async function cachedFetch(url, opts={}) {
  const now = Date.now();
  // Check RAM cache first (CACHE_TTL)
  if (cache.has(url)) {
  ...
  // Fetch fresh from API — share an already-in-flight fetch for the same URL
  if (_inflightFetches.has(url)) return _inflightFetches.get(url);
  ...
  _inflightFetches.set(url, _fetchPromise);
  try { return await _fetchPromise; } finally { _inflightFetches.delete(url); }
}
