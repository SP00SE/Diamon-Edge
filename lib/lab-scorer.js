...
function computePitchTypeEdge(ctx) {
  ...
  // Two-stage scoring:
  // Stage 1 — depth (usage-weighted, small per-pitch scale ±3)
  // Stage 2 — stacking (+2 per pitch type where batter clearly has edge, −2 where clearly weak)
  // Result: handling 1 pitch well ≈ +4; 2 pitch types ≈ +6; all 3+ ≈ +8 (cap ±8)
  var depthScore = 0, stackBonus = 0, pitchesScored = 0;
  var pitches = [];
  ...
  pitcherRows.forEach(function(p) {
    ...
    // BA vs pitch type — depth signal (±3 max per pitch)
    var baPts;
    if      (ba >= 0.310) baPts =  3;   // clearly good vs this pitch
    else if (ba >= 0.280) baPts =  1;
    else if (ba >= 0.240) baPts =  0;   // neutral band
    else if (ba >= 0.210) baPts = -1;
    else if (ba >  0)     baPts = -3;   // clearly struggles vs this pitch
    else return;                         // no BA data — skip

    // Whiff modifier ±1 (small, just tips the scale)
    var whiffMod = bWhiff >= 0.30 ? -1 : bWhiff > 0 && bWhiff < 0.15 ? 1 : 0;

    // Amplifier for elite put-away pitch the batter whiffs on
    var amp = 1.0;
    if (pWhiff >= 0.35 && bWhiff >= 0.30) amp = 1.25;
    else if (pWhiff <= 0.15)              amp = 0.80;

    var pitPts = (baPts + whiffMod) * amp;

    // Stage 1: depth — usage-weighted contribution
    depthScore += pitPts * usage;

    // Stage 2: stacking — +2 per pitch type batter clearly handles, -2 per clearly struggles
    // Threshold: baPts ≥ +3 (BA ≥ .310) = stacks positive; baPts ≤ -3 (BA < .210) = stacks negative
    if (baPts >=  3) stackBonus += 2;
    else if (baPts <= -3) stackBonus -= 2;
    ...
    pitches.push({
      ...
      pWhiff:    pWhiff,
      pts:       pitPts,
      edge:      baPts >=  1 ? 'batter' : baPts <= -1 ? 'pitcher' : 'neutral',
    });
  });

  if (pitchesScored < 2) return { value: 0, hasData: false, pitches: [] };

  var raw = depthScore + stackBonus;
  return {
    value:   clamp(Math.round(raw), -8, 8),
    hasData: true,
