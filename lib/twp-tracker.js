...
const TwpTracker = (() => {
  ...
  // Season-to-date record across the ENTIRE log, no date window. `since` is
  // the earliest tracked day. (A retroactive full-season backtest would need
  // as-of-date stats the MLB API doesn't serve — current aggregates would let
  // the model "know the future" — so the honest season record grows forward
  // from the day tracking began.)
  function computeAllTime(log) {
    let wins = 0, losses = 0, since = null;
    for (const e of Object.values(log.games)) {
      if (e.officialDate && (!since || e.officialDate < since)) since = e.officialDate;
      if (e.status === 'win') wins++;
      else if (e.status === 'loss') losses++;
    }
    const graded = wins + losses;
    return { wins, losses, graded, pct: graded ? wins / graded * 100 : null, since };
  }
  ...
  return { SCHEMA_VERSION, emptyLog, normalizeLog, makeSnapshot, addSnapshot, refreshSnapshot, attachSim, gradeGame, windowDates, computeStats, computeAllTime, classifyMiss, setMissCategory, missPatterns };
})();
